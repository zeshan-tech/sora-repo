...
import { Dialog, DialogContent, DialogFooter, DialogHeader, DialogTitle, DialogTrigger } from "~/components/elements/Dialog";
...
export const meta = mergeMeta(() => [{ title: "Cache Management" }, { name: "description", content: "Cache Management" }]);
...
const CacheAdminRoute = () => {
  ...
  return (
    <motion.div key={location.key} initial={{ x: "-10%", opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: "-10%", opacity: 0 }} transition={{ duration: 0.3 }} className="flex w-full flex-col items-center justify-center px-3 sm:px-5">
      <h2>Cache Management</h2>
      <SearchForm onSubmit={onSubmit} textOnButton="Search" textPlaceHolder="Search cache key" defaultValue={query} />
      <Spacer x={5} />
