...
export const meta = mergeMeta<typeof loader>(({ data, params }) => {
  if (!data) {
    return [{ title: "Missing Movie" }, { name: "description", content: `There is no movie with the ID: ${params.movieId}` }];
  }
  const { detail } = data || {};
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  const [detail, recommendations] = await Promise.all([getMovieDetail(mid), getRecommendation("movie", mid)]);
  ...
  if (provider === "Loklok") {
    ...
    const [movieDetail, imdbRating, fimg] = await Promise.all([loklokGetMovieInfo(idProvider), detail?.imdb_id && process.env.IMDB_API_URL !== undefined ? getImdbRating(detail?.imdb_id) : undefined, fetch(extractColorImageUrl)]);
  ...
  if (provider === "Flixhq") {
    ...
    const [movieStreamLink, imdbRating, fimg] = await Promise.all([getWatchEpisode(idProvider, getId(idProvider)), detail?.imdb_id && process.env.IMDB_API_URL !== undefined ? getImdbRating(detail?.imdb_id) : undefined, fetch(extractColorImageUrl)]);
    const fimgb = Buffer.from(await fimg.arrayBuffer());
  ...
  if (provider === "KissKh") {
    if (!idProvider) throw new Response("Id Not Found", { status: 404 });
    const [episodeDetail, imdbRating, fimg] = await Promise.all([getKissKhInfo(Number(idProvider)), detail?.imdb_id && process.env.IMDB_API_URL !== undefined ? getImdbRating(detail?.imdb_id) : undefined, fetch(extractColorImageUrl)]);
    const fimgb = Buffer.from(await fimg.arrayBuffer());
    const [episodeStream, episodeSubtitle, palette] = await Promise.all([getKissKhEpisodeStream(Number(episodeDetail?.episodes[0]?.id)), episodeDetail?.episodes[0] && episodeDetail?.episodes[0].sub > 0 ? getKissKhEpisodeSubtitle(Number(episodeDetail?.episodes[0]?.id)) : undefined, detail?.backdrop_path || detail?.poster_path ? await Vibrant.from(fimgb).getPalette() : undefined]);
  ...
  const [imdbRating, fimg] = await Promise.all([detail?.imdb_id && process.env.IMDB_API_URL !== undefined ? getImdbRating(detail?.imdb_id) : undefined, fetch(extractColorImageUrl)]);
  const fimgb = Buffer.from(await fimg.arrayBuffer());
  const palette = detail?.backdrop_path || detail?.poster_path ? await Vibrant.from(fimgb).getPalette() : undefined;
  return json(
    {
...
export const handle: Handle = {
  breadcrumb: ({ match, t }) => (
    <>
      <BreadcrumbItem to={`/movies/${match.params.movieId}/`} key={`movies-${match.params.movieId}-overview`}>
        {(match.data as { detail: IMovieDetail })?.detail?.title || match.params.movieId}
      </BreadcrumbItem>
      <BreadcrumbItem to={`/movies/${match.params.movieId}/watch`} key={`movies-${match.params.movieId}-watch`}>
        {t("watch")}
      </BreadcrumbItem>
    </>
  ),
  ...
  miniTitle: ({ match, t }) => ({
    ...
    imageUrl: TMDB.posterUrl((match.data as { detail: IMovieDetail })?.detail?.poster_path || "", "w92"),
  }),
};
...
const MovieWatch = () => {
  ...
  return (
    <div className="mt-3 flex w-full flex-col items-center justify-center px-3 sm:px-0">
      <WatchDetail id={Number(id)} type="movie" title={`${detail?.title} (${releaseYear})`} overview={detail?.overview || ""} posterPath={detail?.poster_path ? TMDB.posterUrl(detail?.poster_path, "w342") : undefined} tmdbRating={detail?.vote_average} imdbRating={imdbRating?.star} genresMedia={detail?.genres} genresMovie={rootData?.genresMovie} genresTv={rootData?.genresTv} recommendationsMovies={recommendations?.items} color={color} />
    </div>
  );
};
