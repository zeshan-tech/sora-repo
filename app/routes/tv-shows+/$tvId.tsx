...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  let nameEng = detail?.original_language === "en" ? detail?.original_name : locale === "en" ? detail?.name : "";
  if (detail?.original_language !== "en" && locale !== "en") {
    const [detailEng, imdbRating, fimg] = await Promise.all([getTvShowDetail(tid, "en-US"), imdbId && process.env.IMDB_API_URL !== undefined ? getImdbRating(imdbId) : undefined, fetch(extractColorImageUrl)]);
  ...
  const [imdbRating, fimg] = await Promise.all([imdbId && process.env.IMDB_API_URL !== undefined ? getImdbRating(imdbId) : undefined, fetch(extractColorImageUrl)]);
  const fimgb = Buffer.from(await fimg.arrayBuffer());
...
export const handle: Handle = {
  ...
  miniTitle: ({ match, t }) => ({
    ...
    imageUrl: TMDB?.posterUrl((match.data as { detail: ITvShowDetail })?.detail?.poster_path || "", "w92"),
  }),
...
const TvShowDetail = () => {
  ...
  const { setBackgroundColor, startChangeScrollPosition } = useHeaderStyle((headerStyle) => headerStyle);
  ...
  const backdropPath = detail?.backdrop_path ? TMDB?.backdropUrl(detail?.backdrop_path || "", "w1280") : undefined;
  ...
  return (
    <>
      ...
      <div className="relative top-[-80px] w-full sm:top-[-200px]">
        <MediaDetail type="tv" item={detail} imdbRating={imdbRating} color={detail.color} trailerTime={currentTime} />
