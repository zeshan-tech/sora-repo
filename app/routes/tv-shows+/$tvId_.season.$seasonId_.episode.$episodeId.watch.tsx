...
import { getImdbRating, getInfoWithProvider, getRecommendation, getTvSeasonDetail, getTvShowDetail, getTvShowIMDBId, getWatchEpisode } from "~/services/tmdb/tmdb.server";
...
export const meta = mergeMeta<typeof loader>(({ data, params }) => {
  if (!data) {
    return [
      ...
      {
        name: "description",
        content: `This season of tv show doesn't have episode ${params.episodeId || ""} or this episode is unavailable`,
      },
    ];
  }
  ...
  return [
    {
      title: `Sora - Watch ${detail?.name || ""} season ${params.seasonId || ""} episode ${params.episodeId || ""}`,
    },
    { name: "description", content: seasonDetail?.overview || detail?.overview || "" },
    {
      name: "keywords",
      content: `Watch ${detail?.name || ""} season ${params.seasonId || ""} episode ${params.episodeId || ""}, Stream ${detail?.name || ""} season ${params.seasonId || ""} episode ${params.episodeId || ""}, Watch ${detail?.name || ""} season ${params.seasonId || ""} episode ${params.episodeId || ""} HD, Online ${detail?.name || ""} season ${params.seasonId || ""} episode ${params.episodeId || ""}, Streaming ${detail?.name || ""} season ${params.seasonId || ""} episode ${params.episodeId || ""}, English, Subtitle ${detail?.name || ""} season ${params.seasonId || ""} episode ${params.episodeId || ""}, English Subtitle`,
    },
    {
      property: "og:url",
    ...
    {
      property: "og:title",
      content: `Sora - Watch ${detail?.name || ""} season ${params.seasonId || ""} episode ${params.episodeId || ""}`,
    },
...
const checkHasNextEpisode = (currentEpisode: number, totalEpisodes: number, totalProviderEpisodes: number) => {
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  if (!tvId || !seasonId || !episodeId || !provider || !idProvider) throw new Response("Not Found", { status: 404 });

  const [detail, imdbId, recommendations, seasonDetail] = await Promise.all([getTvShowDetail(tid), getTvShowIMDBId(tid), getRecommendation("tv", tid), getTvSeasonDetail(tid, sid)]);
  ...
  const extractColorImageUrl = process.env.NODE_ENV === "development" ? TMDB.backdropUrl(detail?.backdrop_path || detail?.poster_path || "", "w300") : `https://corsproxy.io/?${encodeURIComponent(TMDB.backdropUrl(detail?.backdrop_path || detail?.poster_path || "", "w300"))}`;
  ...
  if (provider === "Loklok") {
    ...
    const palette = (detail?.backdrop_path || detail?.poster_path) && fimgb ? await Vibrant.from(fimgb).getPalette() : undefined;
    return json(
      {
        idProvider,
        provider,
  ...
  if (provider === "Flixhq") {
    ...
    const findTvSeason = (tvDetail as IMovieInfo)?.seasons?.find((s) => s.season === Number(season));
    ...
    const tvEpisodeDetail = findTvSeason?.episodes?.find((episode) => episode.season === Number(seasonId) && episode.episode === Number(episodeId));
    const fimgb = Buffer.from(await fimg.arrayBuffer());
    if (tvEpisodeDetail) {
      const [tvEpisodeStreamLink, palette] = await Promise.all([getWatchEpisode(tid.toString(), tvEpisodeDetail.id), detail?.backdrop_path || detail?.poster_path ? await Vibrant.from(fimgb).getPalette() : undefined]);
      return json(
        {
          provider,
          idProvider,
  ...
  if (provider === "KissKh") {
    ...
    const fimgb = Buffer.from(await fimg.arrayBuffer());
    const [episodeStream, episodeSubtitle, palette] = await Promise.all([getKissKhEpisodeStream(Number(episodeSearch?.id)), episodeSearch && episodeSearch.sub > 0 ? getKissKhEpisodeSubtitle(Number(episodeSearch?.id)) : undefined, detail?.backdrop_path || detail?.poster_path ? await Vibrant.from(fimgb).getPalette() : undefined]);
  ...
  const fimgb = Buffer.from(await fimg.arrayBuffer());
  const palette = detail?.backdrop_path || detail?.poster_path ? await Vibrant.from(fimgb).getPalette() : undefined;
  return json({
    idProvider,
    provider,
    detail,
...
export const handle: Handle = {
  breadcrumb: ({ match, t }) => (
    <>
      <BreadcrumbItem to={`/tv-shows/${match.params.tvId}/`} key={`tv-shows-${match.params.tvId}-overview`}>
        {(match.data as { detail: ITvShowDetail })?.detail?.name || (match.data as { detail: ITvShowDetail })?.detail?.original_name || match.params.tvId}
      </BreadcrumbItem>
      <BreadcrumbItem to={`/tv-shows/${match.params.tvId}/season/${match.params.seasonId}/`} key={`tv-shows-${match.params.tvId}-season-${match.params.seasonId}-episodes`}>
        {t("season")} {match.params.seasonId}
      </BreadcrumbItem>
      <BreadcrumbItem to={`/tv-shows/${match.params.tvId}/season/${match.params.seasonId}/episode/${match.params.episodeId}`} key={`tv-shows-${match.params.tvId}-season-${match.params.seasonId}-episode-${match.params.episodeId}`}>
  ...
  miniTitle: ({ match, t }) => ({
    title: (match.data as { detail: ITvShowDetail })?.detail?.name || (match.data as { detail: ITvShowDetail })?.detail?.original_name || "",
    subtitle: `${(match.data as { seasonDetail: ISeasonDetail })?.seasonDetail?.name} ${t("episode")} ${match.params.episodeId}`,
...
const TvEpisodeWatch = () => {
  const { detail, recommendations, imdbRating, seasonDetail, providers, color } = useLoaderData<typeof loader>();
