...
export async function authenticate(request: Request, customAuthRequired?: boolean, botcheckRequired?: boolean, payloadCheckRequired?: boolean, headers = new Headers()) {
  const isbotAuth = isbot.spawn();

  isbotAuth.exclude(["Checkly", "Checkly, https://www.checklyhq.com", "Checkly/1.0 (https://www.checklyhq.com)", "chrome-lighthouse", "googlebot", "googlebot/2.1 (+http://www.google.com/bot.html)", "bingbot", "bingbot/2.0 (+http://www.bing.com/bingbot.htm)", "discordbot", "Discordbot/2.0", "twitterbot", "Twitterbot/1.0", "vercel", "Vercel/1.0 (https://vercel.com/docs/bots)"]);
  ...
  if (botcheck && botcheckRequired) {
    ...
    if (payload !== authToken?.req_payload && process.env.NODE_ENV === "production" && payloadCheckRequired) {
